...
from `northstar-bq.northstar_metrics.v_mrr_month_end_account`
where monthly_recurring_revenue < 0;


-- 2. Plan totals versus company total (with diff)
with
  company_totals as (
    select
        month_end_date
...
, per_plan as (
    select
        month_end_date
      , plan_id
      , sum(monthly_recurring_revenue) as mrr_per_plan
    from `northstar-bq.northstar_metrics.v_mrr_month_end_account`
    group by month_end_date, plan_id
  )
, plan_rollup as (
    select
        month_end_date
      , sum(mrr_per_plan) as plan_total_mrr
    from per_plan
    group by month_end_date
  )
...
select
  ...
  , plan_rollup.plan_total_mrr
  , company_totals.company_total_mrr - plan_rollup.plan_total_mrr as difference_mrr
from company_totals
join plan_rollup
  on company_totals.month_end_date = plan_rollup.month_end_date
order by company_totals.month_end_date;
...
from `northstar-bq.northstar_metrics.v_mrr_month_end_account` snapshot_view
